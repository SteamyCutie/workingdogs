...
const Roadmap = () => {
  ...
  for (let i = 0; i < ROADMAP.length; i ++) {
    ...
    if ( i < ROADMAP.length - 1) {
      phaseMDItems.push(<div key={i} className="mt-[-135px] md:mt-[-101px]">{i % 2 === 0 ? <PhaseMDBrach12 /> : <PhaseMDBrach21 />}</div>)
    }
  }
  return (
    <div className="justify-center mt-8" id="roadmap">
      <div className="w-[100%] h-[1900px] bg-no-repeat bg-contain bg-none lg:bg-[url('./assets/img/roadmap/back.png')]"></div>
      ...
      <div className="flex justify-center">
        ...
        <div className="grid justify-items-center gap-0 lg:hidden">{ phaseMDItems }</div>
      </div>
    </div>
  )
}
